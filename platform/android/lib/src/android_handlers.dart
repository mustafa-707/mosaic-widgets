...
class ContainerHandler extends AndroidNodeHandler {
  ...
  String handle(
    ...
    final bindWantsShape =
        isBgBind && ((node.data['radius'] ?? 0).toDouble() > 0 ||
    ...
    if (hasRoundedBackground) {
      if (idAttr.isEmpty) {
        final viewId = context.uniqueViewId('hw_clip', '${radius}_${widthVal}_$heightVal');
        idAttr = ' android:id="@+id/$viewId"';
        context.registerClippedView(viewId);
      } else {
        final existing =
            RegExp(r'@\+id/(\w+)').firstMatch(idAttr)?.group(1);
...
$gradientTag$cornersTag$stroke
  ...
  String _shadowLayerXml(
    ...
    final fg = RegExp(r'android:background="([^"]+)"').firstMatch(bgAttr)?.group(1);
    String fgRef;
    if (fg == null) {
      ...
      final shape = _shapeDrawableXml(
          context, {'hex': fg, 'opacity': 1.0}, null, radius, corners: corners);
...
<layer-list xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    final ref = context.registerDrawable(
        'hw_shadowed_${_stableHash(layer)}', layer);
