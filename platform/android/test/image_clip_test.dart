...
void main() {
  ...
  test('radius emits rounded corners', () async {
    ...
    final drawable = r.resXmlFiles()
        .firstWhere((f) => f.path.contains('hw_clip_'));
  ...
  test('identical shapes collapse to one drawable', () async {
    final r = await runAndroid([
      irDef({
        '__type': 'HWRow',
        'children': [
          {'__type': 'HWNetworkImage', 'url': 'https://a/1.jpg', 'circle': true},
          {'__type': 'HWNetworkImage', 'url': 'https://a/2.jpg', 'circle': true},
        ],
      })
    ]);
    final clips =
