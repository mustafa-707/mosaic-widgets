...
void main() {
  ...
  group('StackHandler alignment mapping', () {
    ...
    test('alignment bottomTrailing → children carry android:layout_gravity="bottom|end"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment top → children carry android:layout_gravity="top|center_horizontal"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment topLeading → children carry android:layout_gravity="top|start"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment topTrailing → children carry android:layout_gravity="top|end"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment leading → children carry android:layout_gravity="center_vertical|start"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment trailing → children carry android:layout_gravity="center_vertical|end"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment bottomLeading → children carry android:layout_gravity="bottom|start"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
    ...
    test('alignment bottom → children carry android:layout_gravity="bottom|center_horizontal"',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
      ...
      final xml = layout(r);
      expect(xml, contains('android:layout_gravity="bottom|center_horizontal"'));
    });
  ...
  group('StackHandler does not clobber MPositioned children', () {
    ...
    test('mixed stack: non-positioned sibling gets gravity, positioned keeps own',
        () async {
      final r = await runAndroid([
        irDef({
          '__type': 'HWStack',
  ...
  group('AAPT validity', () {
    test('stack with alignment produces AAPT-valid XML (first line is <?xml)',
      ...
      expect(offender, isNull,
          reason:
              'res XML must start with <?xml; offending file: $offender');
    });
  });
}
