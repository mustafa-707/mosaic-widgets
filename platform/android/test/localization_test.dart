...
void main() {
  ...
  test('localized text references a string resource, not a literal', () async {
    final r = await runAndroid([irDef(text({'__type': 'HWLocalized', 'key': 'trending'}))],
  ...
  test('the default locale goes in values/, others in values-<locale>/',
    ...
    expect(r.exists('android/app/src/main/res/values/mosaic_localized.xml'), isTrue);
    expect(r.exists('android/app/src/main/res/values-ar/mosaic_localized.xml'), isTrue);
  ...
  test('resource names are consistent between layout and values', () async {
    final r = await runAndroid([irDef(text({'__type': 'HWLocalized', 'key': 'read'}))],
        config: configWithStrings());
  ...
  test('values are XML-escaped', () async {
    ...
    final r = await runAndroid([irDef(text({'__type': 'HWLocalized', 'key': 'amp'}))],
        config: config);
    expect(r.file('android/app/src/main/res/values/mosaic_localized.xml'),
        contains('Tom &amp;'));
  });
