...
void main() {
  group('adaptive (dark) colors', () {
    test(
      ...
      final light = r.file(
          'android/app/src/main/res/values/mosaic_colors.xml');
      final night = r.file(
      ...
      final resDir = Directory(
          '${r.root.path}/android/app/src/main/res');
      final allRes = resDir
          .listSync(recursive: true)
          .whereType<File>()
          .where((f) =>
              f.path.endsWith('.xml') &&
              !f.path.contains('mosaic_colors'))
  ...
  group('bind colors', () {
    ...
    test('bind color does not crash and produces a valid layout', () async {
      ...
      expect(
          r.exists('android/app/src/main/res/layout/hw_testw.xml'), isTrue);
