...
void main() {
  ...
  group('drawables', () {
    ...
    test('flags a referenced drawable that does not exist', () {
      ...
      final problems = validateDrawables([defWithIcon('ic_bitcoin')], root.path);
    ...
    test('accepts a drawable present in any drawable* qualifier dir', () {
      write('android/app/src/main/res/drawable-night/ic_bitcoin.xml', '<vector/>');
      expect(validateDrawables([defWithIcon('ic_bitcoin')], root.path), isEmpty);
    });
  ...
  group('host plugin wiring', () {
    ...
    test('flags a plugin file missing from the Runner target', () {
      writeValidAppGroup();
      write('ios/Runner/AppDelegate.swift', 'MosaicPlugin.register(with: self)');
      write('ios/Runner/MosaicPlugin.swift', 'class MosaicPlugin {}');
      write('ios/Runner.xcodeproj/project.pbxproj', 'AppDelegate.swift');
      final problems = validateProject(config(), root.path);
      expect(problems.single.message, contains('not a member of the Runner target'));
    });
