...
class AddWidgetCommand extends Command {
  ...
  @override
  final description =
      'Adds a widget, live activity or control definition.';
...
abstract class _AddNamedCommand extends Command {
  ...
  Future<void> run() async {
    ...
    final configContent =
        (configFile?.existsSync() ?? false) ? configFile!.readAsStringSync() : null;
...
String? insertUnderKey(
  ...
  final match =
      RegExp('^$key:[ \\t]*\$', multiLine: true).firstMatch(yaml);
  if (match == null) {
