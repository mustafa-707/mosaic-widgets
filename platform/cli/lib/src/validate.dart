...
List<Problem> validateDrawables(
  ...
  bool exists(String name) => drawableDirs.any((dir) =>
      extensions.any((ext) => File(p.join(dir.path, '$name.$ext')).existsSync()));
...
List<Problem> validateStringKeys(
  ...
  final problems = <Problem>[
    for (final key in referenced.where((k) => !fallback.containsKey(k)).toList()
      ..sort())
      Problem(
...
List<Problem> validateProject(MosaicConfig config, String projectRoot) {
  ...
  final mainActivity = _findMainActivity(projectRoot, config.app.androidPackage);
