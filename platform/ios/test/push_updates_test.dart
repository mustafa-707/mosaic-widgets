...
void main() {
  test('emits a push handler when opted in', () async {
    final r = await runIos([irDef(text('hi'))], config: configWithPush(push: true));
    ...
    expect(swift, contains('func pushTokenDidChange(_ pushInfo: WidgetPushInfo, widgets: [WidgetInfo])'));
  });
  ...
  test('nothing is emitted when push is off', () async {
    final r = await runIos([irDef(text('hi'))], config: configWithPush(push: false));
    final swift = r.swiftForTestW();
  ...
  test('a configurable widget also gets the handler', () async {
    final r = await runIos([
      IRDefinition.fromJson({
        ...
        'root': text('hi'),
        'params': [
          {'key': 'city', 'label': 'City', 'type': 'text', 'defaultValue': 'London'},
        ],
      })
  ...
  test('the host plugin exposes the tokens to Dart', () async {
    ...
    expect(plugin, contains('tokens[String(key.dropFirst(prefix.count))] = token'));
