...
void main() {
  test('a flexible child claims the free space', () async {
    ...
    expect(swift, contains('.frame(maxWidth: .infinity, maxHeight: .infinity)'));
  });
  ...
  test('flex ratios are not faked with GeometryReader arithmetic', () async {
    ...
    expect('.frame(maxWidth: .infinity, maxHeight: .infinity)'
        .allMatches(swift).length, greaterThanOrEqualTo(2));
  });
}
