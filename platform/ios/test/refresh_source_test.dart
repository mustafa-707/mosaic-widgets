...
void main() {
  ...
  test('emits the file with no sources when none are declared, so the '
      'intents still compile', () async {
    final r = await runIos([irDef(text('hi'))]);
    expect(r.exists(sourcesPath), isTrue);
    final swift = readFile(r.file(sourcesPath));
    expect(
        swift, contains('static let all: [String: [MosaicRefreshSource]] = [:]'));
  });
  ...
  group('diagnostics', () {
    test('stamps a status on every attempt, so an unchanged payload is still '
        'distinguishable from a refresh that never ran', () async {
      final r = await runIos([irDef(text('hi'))], config: configWithRefresh());
      final swift = readFile(r.file(sourcesPath));
      expect(swift, contains('forKey: "mosaic_refresh_status"'));
      expect(swift, contains('recordStatus(wroteAny ? "ok" : (failure ?? "no data"))'));
    });
    ...
    test('bypasses the URL cache, which would otherwise return an identical '
  ...
  test('the callback intent fetches, then reloads', () async {
    ...
    final body = intents.substring(intents.indexOf('struct MosaicCallbackIntent'));
  ...
  test('uses an ephemeral session, not URLSession.shared, which an extension '
  ...
  test('falls back to the pending-callback handoff when the fetch does not '
