...
**Routing a widget tap:** `await MosaicBridge.initialDeepLink()` in `main()` — a tap that cold-launches the app is delivered before anything can subscribe to `onDeepLink`, so read it once for the initial route. It clears itself.

**MRaw — verbatim native code:** `MRaw(swift: …, androidXml: …, binds: ['key'])`. The escape hatch for anything the DSL cannot express: your SwiftUI / layout XML is inserted unchanged while Mosaic still generates the App Group wiring, timeline and update path. **Declare every bind key you read** — the generator cannot parse a snippet, so an undeclared key is never fetched and the view renders empty with no error. Nothing inside is validated; on Android a non-whitelisted class fails at run time as "Can't load widget".

**MAdaptive — different subtree per platform:** `MAdaptive(ios: …, android: …)`. Chosen at *generation* time; each generator emits only its branch. Cannot branch at runtime, on OS version or API level — the builder runs once and feeds one tree to both. Reach for it last, after `sfSymbol`/`androidDrawable` and `compactRoot`; it earns its place where the DSL already documents divergence (`MFlipper` first-child-only on iOS, `MActivityIndicator` animating only on Android, `MFlexible` ratios Android-only).
