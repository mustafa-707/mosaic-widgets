...
- **Bind defaults**: `MBind('price', defaultValue: '64000')` — what renders before the app has ever written the key.
  - Without one a bind shows `--`. That is worst where it is most visible: the **OS widget gallery** renders `placeholder(in:)` before your app has run at all, so a widget with no defaults previews as a wall of `--` while the user decides whether to add it.
  - iOS seeds `placeholder(in:)` from the declared defaults; Android passes them to `MosaicData.resolveString(key, fallback)`. `MDeviceValue` accepts one too, for the window before the first native read.
