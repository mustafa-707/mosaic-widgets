...
void main() {
  test('renders from the cache with a placeholder fallback', () async {
    ...
    expect(swift,
        contains('MosaicImageCache.cached("https://cdn.example.com/a.jpg")'));
  ...
  test('the provider prefetches literal urls', () async {
    final r = await runIos([
      irDef(
          {'__type': 'HWNetworkImage', 'url': 'https://cdn.example.com/a.jpg'})
    ]);
  ...
  test(
      'a bound url is prefetched from stored data, so a url that arrived in '
