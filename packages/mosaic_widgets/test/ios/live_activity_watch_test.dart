...
void main() {
  test('off by default, so 16.1 devices keep their Live Activities', () async {
    final r = await runIos([],
        config: _config(watch: false), liveActivities: [_activity()]);
    final swift =
        readFile(r.file('ios/HomeWidgetExtension/DeliveryLiveActivity.swift'));
  ...
  test('opting in emits the modifier and raises that activity to 18', () async {
    final r = await runIos([],
        config: _config(watch: true), liveActivities: [_activity()]);
    final swift =
        readFile(r.file('ios/HomeWidgetExtension/DeliveryLiveActivity.swift'));
  ...
  test('the modifier goes on the configuration, not inside the island',
      () async {
