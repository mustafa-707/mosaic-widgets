...
void main() {
  ...
  test('one Localizable.strings per locale, beside the generated Swift',
    ...
    expect(
        readFile(
            r.file('ios/HomeWidgetExtension/en.lproj/Localizable.strings')),
        contains('"trending" = "TRENDING NOW";'));
    expect(
        readFile(
            r.file('ios/HomeWidgetExtension/ar.lproj/Localizable.strings')),
        contains('الأكثر تداولاً'));
  });

  test('quotes in values are escaped so the strings file stays valid',
      () async {
