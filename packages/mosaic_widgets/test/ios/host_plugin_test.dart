...
void main() {
  ...
  test('emits the host plugin into ios/Runner', () async {
    ...
    expect(
        swift, contains('public class MosaicPlugin: NSObject, FlutterPlugin'));
  ...
  test('falls back to the configured App Group so setAppGroupId is optional',
    ...
    final swift = readFile(r.file(pluginPath));
    expect(swift,
        contains('public static let appGroup = "group.com.acme.app.widgets"'));
  ...
  group('live activities', () {
    ...
    test(
        'omits controller references when none are declared, so the file '
      ...
      expect(swift,
          contains('case "startActivity", "updateActivity", "endActivity":'));
    });
  });
}
