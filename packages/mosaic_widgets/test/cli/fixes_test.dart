...
import 'package:mosaic_widgets/src/cli/fixes.dart';
...
$extra''');
  ...
  MosaicConfig config() => MosaicConfig.fromYaml(read('mosaic.yaml'));
  ...
  group('entitlements', () {
    test('are created for targets that exist', () {
      ...
      expect(exists('ios/HomeWidgetExtension/HomeWidgetExtension.entitlements'),
          isTrue);
    });
...
''');
      ...
      fix();
      expect(
          read('ios/Runner/Info.plist'), contains('NSSupportsLiveActivities'));
    });
