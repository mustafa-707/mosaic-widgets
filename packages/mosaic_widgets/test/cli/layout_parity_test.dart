...
void main() {
  ...
  Future<({bool android, bool ios})> fillsMainAxis(
    ...
    final swift = File(p.join(root.path, 'ios/HomeWidgetExtension/TestW.swift'))
        .readAsStringSync();
    ...
    final stack =
        swift.substring(start, swift.indexOf('}', swift.indexOf('sibling')));
    final iosFills =
  ...
  Future<({String xml, String kotlin, String swift})> both(
    ...
    return (
      xml: File(
              p.join(root.path, 'android/app/src/main/res/layout/hw_testw.xml'))
          .readAsStringSync(),
  ...
  group('a property honoured on one platform is honoured on the other', () {
    ...
    test('a circle clip is a circle on both, not a rounded rect', () async {
      ...
      final clip =
          Directory(p.join(root.path, 'android/app/src/main/res/drawable'))
              .listSync()
              .whereType<File>()
    ...
    test('a count-down timer counts down on both', () async {
      ...
      expect(
          g.kotlin, contains('setChronometerCountDown(R.id.hw_timer_0, true)'));
    ...
    test('a count-up timer counts up on both', () async {
      ...
      expect(g.kotlin,
          contains('setChronometerCountDown(R.id.hw_timer_0, false)'));
