...
void main() {
  ...
  test('previewLayout renders the real widget in the picker', () async {
    ...
    expect(
        r.file(infoPath), contains('android:previewLayout="@layout/hw_testw"'));
  });
  ...
  test('description is emitted as a string resource reference, never a literal',
    ...
    final strings =
        r.file('android/app/src/main/res/values/mosaic_strings.xml');
    expect(
        strings,
        contains(
            '<string name="mosaic_desc_testw">Live weather for your city</string>'));
  });
