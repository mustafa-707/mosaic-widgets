...
void main() {
  ...
  test('a network image does not collapse inside a row', () async {
    final xml =
        await inRow({'__type': 'HWNetworkImage', 'url': 'https://a/b.jpg'});
  ...
  test('no generated layout puts match_parent height inside a row', () async {
    ...
    const candidates = [
      {'__type': 'HWNetworkImage', 'url': 'https://a/b.jpg'},
      {
        '__type': 'HWStack',
        'children': [
          {'__type': 'HWText', 'text': 'x'}
        ]
      },
      {
        '__type': 'HWCenter',
        'child': {'__type': 'HWText', 'text': 'x'}
      },
      {
        '__type': 'HWFlexible',
        'flex': 1,
    ...
    for (final c in candidates) {
      ...
      final body =
          xml.substring(xml.indexOf('>', xml.indexOf('<FrameLayout')) + 1);
      final offenders = RegExp(r'android:layout_height="match_parent"')
          .allMatches(body)
          .length;
      expect(offenders, 0,
          reason: 'match_parent height under a row for ${c['__type']}');
    }
  });
}
