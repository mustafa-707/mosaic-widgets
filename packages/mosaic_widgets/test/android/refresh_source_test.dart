...
void main() {
  ...
  test('emits the sources table with every declared source', () async {
    final r =
        await runAndroid([irDef(text('hi'))], config: configWithRefresh());
  ...
  test(
      'emits the file with no sources when none declared, so providers still '
      'compile', () async {
  ...
  test(
      'stamps a status so an unchanged payload is distinguishable from a '
  ...
  group('provider wiring', () {
    test(
        'the callback broadcast fetches off the main thread and keeps the '
      ...
      final branch =
          kt.substring(kt.indexOf('intent.action == mosaicCallbackAction'));
