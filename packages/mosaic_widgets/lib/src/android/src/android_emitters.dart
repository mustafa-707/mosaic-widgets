    ...
    final entries = config.refresh.entries.map((e) {
      final sources = e.value.map((source) {
        final headers = source.headers.entries
            .map(
                (h) => '"${kotlinEscape(h.key)}" to "${kotlinEscape(h.value)}"')
            .join(', ');
        final map = source.map.entries
            .map(
                (m) => '"${kotlinEscape(m.key)}" to "${kotlinEscape(m.value)}"')
            .join(', ');
...
''');
  ...
  Future<void> _generateHostPlugin(String projectRoot) async {
    ...
    // Emitted only when a channel is declared, so a phone-only app carries no
    // reference to a class that was never generated.
    final tvPublishBody = config.tvChannels.isEmpty
        ? '''                    result.error("UNSUPPORTED",
                        "No tv_channels declared in mosaic.yaml", null)'''
...
''');
  ...
  /// Emits `MosaicTv.kt` — the Android TV home-screen channel publisher.
  ///
  /// Deliberately not a widget. The Android TV and Google TV launchers host no
  /// AppWidgets at all, so a RemoteViews tree never appears there however it is
  /// declared. The TV home screen shows *channels* of preview programs through
  /// `TvProvider`, and the launcher draws those cards itself — meaning the DSL
  /// has nothing to render and what a channel needs is content.
  ///
  /// Emitted only when `tv_channels:` is declared, because it pulls in the
  /// androidx.tvprovider dependency and a WRITE_EPG_DATA permission that a
  /// phone-only app should not carry.
  Future<void> _generateTvChannels(String projectRoot) async {
    final channels = config.tvChannels;
    ...
    final declared = channels
        .map((c) =>
            '"${kotlinEscape(c.name)}" to "${kotlinEscape(c.displayName)}"')
        .join(',\n        ');

    await File(p.join(kotlinDir.path, 'MosaicTv.kt'))
        .writeAsString('''$kotlinSentinel
package ${config.app.androidPackage}.mosaic_generated
