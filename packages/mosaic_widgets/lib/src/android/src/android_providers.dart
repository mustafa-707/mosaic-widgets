...
extension AndroidProviderEmitters on AndroidGenerator {
  Future<void> _generateKotlinProvider(
    ...
    final imageBinds =
        usedBinds.entries.where((e) => e.value.contains('image')).map((e) => (
    ...
    final a11yLogic = _contentDescriptions
        .map((e) => '        views.setContentDescription(R.id.${e.viewId}, '
...
$fillBlock                canvas.drawPath(path, paint)
    ...
    final hasRefresh = buttons.any((b) => b['__type'] == 'HWRefreshAction');
    final refreshKeys =
